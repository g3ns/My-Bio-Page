class DiscordUploader {
    constructor() {
        ...
        this.WEBHOOK_NORMAL_LIMIT = 8 * 1024 * 1024;  // 8MB limit for normal webhooks
        this.WEBHOOK_PREMIUM_LIMIT = 50 * 1024 * 1024; // 50MB limit for premium webhooks
        this.BOT_UPLOAD_LIMIT = 25 * 1024 * 1024;     // 25MB limit for bot uploads
        this.CHUNK_SIZE = 8 * 1024 * 1024;            // Default to 8MB chunks
        ...
        this.USE_BOT_UPLOADER = false;                // Whether to use the bot uploader
    ...
    validateWebhookUrl(url) {
        ...
        // Check if URL is for a bot channel (simplified for demo)
        const botChannelRegex = /^https:\/\/(discord\.com|discordapp\.com)\/api\/webhooks\/[0-9]+\/[A-Za-z0-9_-]+\?bot=true$/;
        
        // Add visual indicator for validation result
        const webhookUrlInput = document.getElementById('webhookUrl');
        const validationMessage = document.querySelector('.validation-message');
        
        // Check if it's a bot channel URL
        if (url && botChannelRegex.test(url)) {
            this.USE_BOT_UPLOADER = true;
            this.CHUNK_SIZE = this.BOT_UPLOAD_LIMIT;
            
            if (webhookUrlInput) webhookUrlInput.classList.remove('input-error');
            if (validationMessage) {
                validationMessage.textContent = 'Using Discord Bot Uploader for large files';
                validationMessage.classList.add('info');
                validationMessage.classList.add('visible');
            }
            return true;
        } 
        // Check if it's a standard webhook URL
        else if (url && discordWebhookRegex.test(url)) {
            this.USE_BOT_UPLOADER = false;
            this.CHUNK_SIZE = this.WEBHOOK_NORMAL_LIMIT;
    ...
    // Detect premium webhook
    async detectPremiumWebhook(webhookUrl) {
        try {
            // Send a test request to check if we can get webhook info
            ...
            if (response.ok) {
                ...
                // Check if the webhook is in a premium guild
                // This is a simplified check - in a real implementation,
                // you'd need to check premium status through Discord's API
                if (data.premium_tier && data.premium_tier >= 2) {
                    this.CHUNK_SIZE = this.WEBHOOK_PREMIUM_LIMIT;
                    return true;
                }
            }
            return false;
        } catch (error) {
            console.log("Error detecting premium webhook:", error);
            return false;
        }
    }
    ...
    // Split file into chunks based on size and webhook type
    splitFileIntoChunks(file) {
        const chunks = [];
        let start = 0;
        
        // Determine appropriate chunk size based on file size and webhook type
        let effectiveChunkSize = this.CHUNK_SIZE;
        
        // If file is small enough for direct upload, use a single chunk
        if (file.size <= this.WEBHOOK_NORMAL_LIMIT) {
            return [file]; // Return the whole file as one chunk
        } 
        // If we're using bot uploader and file is within bot limit
        else if (this.USE_BOT_UPLOADER && file.size <= this.BOT_UPLOAD_LIMIT) {
            return [file]; // Return the whole file for bot upload
        }
        // If we have a premium webhook and file is within premium limit
        else if (file.size <= this.WEBHOOK_PREMIUM_LIMIT && this.CHUNK_SIZE === this.WEBHOOK_PREMIUM_LIMIT) {
            return [file]; // Return the whole file for premium webhook
        }
        // Otherwise, split into appropriate chunks
        else {
            // If file is too large for webhook but we have bot uploader, use bot chunk size
            if (this.USE_BOT_UPLOADER && file.size > this.WEBHOOK_PREMIUM_LIMIT) {
                effectiveChunkSize = this.BOT_UPLOAD_LIMIT;
            }
    ...
    async uploadChunk(chunk, filename, webhookUrl, chunkIndex, retryCount = 0) {
        ...
        try {
            // Check if we should use bot uploader for this chunk
            if (this.USE_BOT_UPLOADER && chunk.size > this.WEBHOOK_NORMAL_LIMIT) {
                return await this.uploadChunkWithBot(chunk, filename, webhookUrl, chunkIndex);
            }
    ...
    // Upload a chunk using the Discord Bot
    async uploadChunkWithBot(chunk, filename, webhookUrl, chunkIndex, retryCount = 0) {
        try {
            // Strip any bot parameter from webhook URL if present
            const cleanWebhookUrl = webhookUrl.replace(/\?bot=true$/, '');
            
            // Create a special payload for the bot uploader
            const formData = new FormData();
            formData.append('file', chunk, filename);
            formData.append('webhook_url', cleanWebhookUrl);
            formData.append('chunk_index', chunkIndex);
            formData.append('use_bot', 'true');
            
            // Send to the bot upload endpoint
            const botUploaderUrl = '/DiscordBotUploader/upload'; // Adjust path as needed
            
            this.showStatus(`Using Discord Bot for large file upload (chunk ${chunkIndex + 1})...`);
            
            const response = await fetch(botUploaderUrl, {
                method: 'POST',
                body: formData
            });
            ...
            if (!response.ok) {
                if (response.status === 413) {
                    throw new Error('File is too large even for bot upload');
                } else if (retryCount < this.MAX_RETRIES) {
                    this.showStatus(`Bot upload failed with status ${response.status}. Retrying (${retryCount + 1}/${this.MAX_RETRIES})...`);
                    await new Promise(resolve => setTimeout(resolve, this.RETRY_DELAY));
                    return this.uploadChunkWithBot(chunk, filename, webhookUrl, chunkIndex, retryCount + 1);
                } else {
                    throw new Error(`Bot upload failed with status ${response.status}`);
                }
            }
            ...
            if (retryCount < this.MAX_RETRIES) {
                this.showStatus(`Bot upload failed: ${error.message}. Retrying (${retryCount + 1}/${this.MAX_RETRIES})...`);
                await new Promise(resolve => setTimeout(resolve, this.RETRY_DELAY));
                return this.uploadChunkWithBot(chunk, filename, webhookUrl, chunkIndex, retryCount + 1);
            } else {
                throw new Error(`Bot upload failed: ${error.message}`);
            }
        }
    }
    ...
    async startUpload() {
        ...
        // Check if file is large and if we should use premium webhook
        if (file.size > this.WEBHOOK_NORMAL_LIMIT && file.size <= this.WEBHOOK_PREMIUM_LIMIT) {
            this.showStatus('Checking webhook type for larger file...');
            const isPremium = await this.detectPremiumWebhook(webhookUrl);
            
            if (isPremium) {
                this.showStatus('Premium webhook detected. Using higher upload limit.');
            } else if (!this.USE_BOT_UPLOADER) {
                this.showStatus('File is larger than 8MB. Will split into chunks.');
            }
        }
        
        // Show appropriate message based on file size and upload method
        if (file.size > this.BOT_UPLOAD_LIMIT && this.USE_BOT_UPLOADER) {
            this.showStatus('File is larger than 25MB. Will be split into chunks and uploaded via bot.');
        } else if (file.size > this.WEBHOOK_PREMIUM_LIMIT && !this.USE_BOT_UPLOADER) {
            this.showStatus('File is larger than 50MB. Will be split into chunks.');
        }
